...
def create_strm(folder, title, link, db_id=''):
    info = Untangle(title)
    folder = os.path.join(folder, info.clean_title)
    if not os.path.exists(folder):
        os.makedirs(folder)
    folder = os.path.join(folder, info.clean_title + '.strm')
    with open(folder, "w") as text_file:
        text_file.write(link)
    if len(db_id) == 0:
        db_id = info.imdb_id
    if len(db_id) > 0:
        with open(folder.replace(".strm", ".nfo"), "w") as text_file:
            text_file.write("http://www.imdb.com/title/%s/" % db_id)
    del info
...
def find_imdb(title=None):
    imdb = storage.Storage.open('imdb')
    if title is None:
        imdb.clear()
        result = ''
    else:
